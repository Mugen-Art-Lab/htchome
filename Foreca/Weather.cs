    ...
    public class Weather : IWeatherProvider
    {
        ...
        public List<WeatherClockWidget.Domain.CityLocation> GetLocation(string Query)
        {
            ...
            try
            {
                ...
                #region Получение языковых Cookie
                if (!isEng)
                {
                ...
                string resultEn = new StreamReader(((HttpWebResponse)request.GetResponse()).GetResponseStream(), Encoding.UTF8).ReadToEnd();
                if (response != null)
                {
                    response.Close();
                    response = null;
                ...
                #region Получение списка из результата
                Regex reList = new Regex("<li id=\"(\\d+)\"><strong>([^,]+),</strong>([^,]+),([^<]+)</li>", RegexOptions.IgnoreCase);
                if (reList.IsMatch(result) && reList.IsMatch(resultEn))
                    for (int mc = 0; mc < reList.Matches(result).Count; mc++)
                    {
                        Match m = reList.Matches(result)[mc];
                        Match mEn = reList.Matches(resultEn)[mc];
                        CityLocationLsit.Add(new CityLocation()
                        {
                            Code = String.Format("{0}/{1}/{2}|{0}/{2}|{3}", mEn.Groups[4].Value.Trim().Replace(" ", "_"), mEn.Groups[3].Value.Trim().Replace(" ", "_"), mEn.Groups[2].Value.Trim().Replace(" ", "_"), m.Groups[2].Value.Trim()),
                            City = String.Format("{0} - {1}/{2}", m.Groups[2].Value.Trim(), m.Groups[4].Value.Trim(), m.Groups[3].Value.Trim())
                        });
                    }
                #endregion
            }
        ...
        public WeatherClockWidget.Domain.WeatherReport GetWeatherReport(string locale, string locationcode, int degreeType)
        {
            ...
            try
            {
                ...
                Uri url = new Uri(GetLanguageUrl(Widget.LocaleManager.LocaleCode));
                String locName = locationcode.Split('|')[2];
                Uri hostUrl = new Uri(url.Scheme + "://" + url.Host + ":" + url.Port.ToString() + "/" + locationcode.Split('|')[1]);
                ...
                #endregion
                #region Некторые населенные пункты работают только путями в 2 уровня
                if (locName.Trim().ToLower() != report.Location.ToLower())
                {
                    hostUrl = new Uri(url.Scheme + "://" + url.Host + ":" + url.Port.ToString() + "/" + locationcode.Split('|')[0]);
                    CurentDataUrl = hostUrl.ToString() + "?" + (isMetric ? "quick_units=metric" : "quick_units=us") + (url.Query != String.Empty ? "&" + url.Query.Substring(1) : String.Empty);
                    CurentData = Helper.HtmlDecode(HTCHome.Core.GeneralHelper.GetXml(CurentDataUrl));
                    #region Название населенного пункта
                    CityNameRe = new Regex("<h1 class=\"entry-title\">([^<]+)</h1>", RegexOptions.IgnoreCase);
                    if (CityNameRe.IsMatch(CurentData))
                        report.Location = CityNameRe.Match(CurentData).Groups[1].Value.Trim();
                    else
                    {
                ...
                #endregion
                #region Прогноз
                String ForecaDataUrl = hostUrl.ToString() + "?tenday&" + (isMetric ? "quick_units=metric" : "quick_units=us") + (url.Query != String.Empty ? "&" + url.Query.Substring(1) : String.Empty);
