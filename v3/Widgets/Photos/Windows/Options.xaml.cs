...
namespace Photos.Windows
{
    ...
    public partial class Options : Window
    {
        ...
        private void WindowSourceInitialized(object sender, EventArgs e)
        {
            ...
            SwitchIntervalSlider.Value = App.Settings.SwitchInterval;
            SwitchIntervalValueTextBlock.Text = SwitchIntervalSlider.Value + " " + Properties.Resources.OptionsIntervalMinutes;
            ...
            PicPathBox.Text = App.Settings.PicsFolder;
            SwitchRandomCheckBox.IsChecked = App.Settings.SwitchRandom;
            SwitchAutomaticallyCheckBox.IsChecked = App.Settings.SwitchAutomatically;
        ...
        private void ApplySettings()
        {
            ...
            App.Settings.SwitchRandom = (bool)SwitchRandomCheckBox.IsChecked;

            if (App.Settings.SwitchAutomatically != (bool)SwitchAutomaticallyCheckBox.IsChecked)
            {
                App.Settings.SwitchAutomatically = (bool) SwitchAutomaticallyCheckBox.IsChecked;
                restartRequired = true;
            }

            if (Directory.Exists(PicPathBox.Text))
            {
                if (App.Settings.PicsFolder != PicPathBox.Text)
                    restartRequired = true;
                App.Settings.PicsFolder = PicPathBox.Text;
            }
            ...
            if (App.Settings.SwitchInterval != SwitchIntervalSlider.Value)
            {
                App.Settings.SwitchInterval = SwitchIntervalSlider.Value;
                restartRequired = true;
            }
            ...
            if (App.Settings.Autostart != (bool)AutostartCheckBox.IsChecked)
            {
                ...
                if (App.Settings.Autostart)
                {
                    try
                    {
                        ...
                        {
                            key.SetValue("Photos Widget (HTC Home)", "\"" + Assembly.GetExecutingAssembly().Location + "\"", RegistryValueKind.String);
                            key.Close();
                        }
                    }
                    catch { }
                ...
                else
                {
                    try
                    {
                        using (RegistryKey key = Registry.CurrentUser.OpenSubKey("Software", RegistryKeyPermissionCheck.ReadWriteSubTree).OpenSubKey("Microsoft").OpenSubKey("Windows").OpenSubKey("CurrentVersion").OpenSubKey("Run", true))
                        {
                            key.DeleteValue("Photos Widget (HTC Home)", false);
                            key.Close();
                        }
                    }
                    catch { }
        ...
        private void PicPathBoxTextChanged(object sender, TextChangedEventArgs e)
        {
            ApplyButton.IsEnabled = true;
        }
        ...
        private void ChangeButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            if (d.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                PicPathBox.Text = d.SelectedPath;
            }
        }

        private void SwitchIntervalSliderValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            ApplyButton.IsEnabled = true;
            SwitchIntervalValueTextBlock.Text = SwitchIntervalSlider.Value + " " + Properties.Resources.OptionsIntervalMinutes;
        }

        //private void RotationSliderValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        //{
        //    ApplyButton.IsEnabled = true;
        //    RotationValueTextBlock.Text = RotationSlider.Value - 30 + "°";
        //}
